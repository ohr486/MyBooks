    ...
    def encode(self, text, clean=False):
        ...
        def checkkigou(x):
            ...
            if len(x) == 1 and len(e) == 2:
                c = (int(e[0])<<8) + int(e[1])
                if (c >= 0xc2a1 and c <= 0xc2bf) or (c >= 0xc780 and c <= 0xcc783) or \
        ...
        while pos < len(text):
            ...
            for e in range(end, pos, -1):
                wd = text[pos:e]
                if wd in self.swe:
                    if wd[0] == '<' and len(wd) > 2: # 特殊トークン
                        kouho = [(self.swe[wd], e)]
                        break
                    else:
                        kouho.append((self.swe[wd], e)) # 通常トークン
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_process", help="process num", default=8)
    parser.add_argument("--combine", help="Concatenate file with <|endoftext|> separator into chunks of this minimum size", type=int, default=50000)
    parser.add_argument("--clean_text", action="store_true")
    parser.add_argument("--tmpsize", help="num chunks in tempolary file", type=int, default=5000)
